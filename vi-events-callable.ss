...
(define events-not-sharing-a-date-with-an-email
  (filter-not
   (λ (event-date) (ormap
                    (λ (email-date) (equal?  event-date email-date))
...
(define events-header-file "OUT/vi-events-header-file.svg")

(define (header-row) 
  (include-template "templates/svg/events-header-static.svg"))

(call-with-output-file events-header-file 
...
(define (start request) 
  (frame-template events-header-file events-file-svg))
